...
func GenerateBatchPacket(ss *SS, flowIndex int, isElephant bool, isNew bool) int {
	...
	ss.FlowMap[flowIndex].RemainingPackets -= uint64(count)
	return count
}
...
func PacketGenerator(ss *SS) {
	...
	var flowId uint64 = 1
	for ss.PacketAmount < PPS && flowId < ss.FlowCount-CPS {
...
func main() {
	...
	for i := 0; i < len(threshold); i++ {
		...
		go func(ss *SS) {
			...
			ss.FlowMap = append(ss.FlowMap, &Flow{false, 0, 0, 0})
			ss.FlowCount = 1
